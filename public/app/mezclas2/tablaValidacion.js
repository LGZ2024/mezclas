...
async function fechTbSolicitadas() {
  try {
    const response = await fetch('/api/mezclas/solicitadas/Validacion')
    if (!response.ok) {
    ...
    mostrarMensaje('Error fetching solicitudes:', 'error')
    throw error
  }
}
const iniciarValidacion = async () => {
  try {
    ...
    if (data.length === 0) {
      mostrarMensaje({
        msg: 'No hay solicitudes para validar'
      })
      hideSpinner()
    }

    // Destruir tabla existente si existe
    if ($.fn.DataTable.isDataTable('#tbValidacion')) {
      $('#tbValidacion').DataTable().destroy()
      $('#tbValidacion tbody').empty()
    }

    // Inicializar nueva tabla con los datos
    $('#tbValidacion').DataTable({
    ...
    hideSpinner()
    mostrarMensaje('Error al iniciar solicitudes')
