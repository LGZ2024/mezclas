...
const iniciarPendiente = async () => {
  try {
    ...
    const table = $('#tbPendientes').DataTable()
...
const verPendiente = async () => {
  try {
    ...
    $('#tbPendientes').DataTable({
      ...
      order: [[0, 'desc']],
      responsive: true,
      dom: '<"d-flex justify-content-between m-4"fl>t<"d-flex justify-content-between "ip>',
      pageLength: 10,
...
const establecerValoresSolicitud = async ({ data }) => {
  ...
  if (document.getElementById('rol').value === 'mezclador' || document.getElementById('rol').value === 'administrativo' || document.getElementById('rol').value === 'master') {
...
export { iniciarPendiente, verPendiente }
