...
async function fechTbSolicitadas () {
  try {
    const response = await fetch('/api/mantenimiento/Preventivo')
    if (!response.ok) {
...
const iniciarPreventivo = async () => {
  try {
    const data = await obtenerTickets()
    const table = $('#tbPreventivo').DataTable()
    console.log(data)
...
const verPreventivo = async () => {
  $('#tbPreventivo').DataTable({
    paging: true,
    order: [[0, 'desc']],
    responsive: true,
...
// Exportar funciones
export { iniciarPreventivo, verPreventivo }
