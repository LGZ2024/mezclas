...
async function fechTbSolicitadas () {
  try {
    const response = await fetch('/api/mantenimiento/Correctivo')
    if (!response.ok) {
...
const iniciarCorrectivo = async () => {
  try {
    const data = await obtenerTickets()
    const table = $('#tbCorrectivo').DataTable()
    console.log(data)
...
const verCorrectivo = async () => {
  $('#tbCorrectivo').DataTable({
    paging: true,
    order: [[0, 'desc']],
    responsive: true,
...
// Exportar funciones
export { iniciarCorrectivo, verCorrectivo }
