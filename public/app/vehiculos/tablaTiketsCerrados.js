...
async function fechTbSolicitadas () {
  try {
    ...
    const response = await fetch(`/api/solicitudServicio/estado/${estado}`)
    if (!response.ok) {
...
const iniciarTicketsCerrados = async () => {
  try {
    const data = await obtenerTickets()
    const table = $('#tbTicketsCerrados').DataTable()
    console.log(data)
...
const verTicketsCerrados = async () => {
  $('#tbTicketsCerrados').DataTable({
    paging: true,
    order: [[0, 'desc']],
    responsive: true,
...
// Exportar funciones
export { iniciarTicketsCerrados, verTicketsCerrados }
