...
async function fetch (status) {
  try {
    const response = await fetch(`/api/devoluciones/${status}`)
    if (!response.ok) {
...
const iniciarProceso = async ({ status }) => {
  try {
    const data = await obtenerProceso(status)
...
const obtenerProceso = async (status) => {
  try {
