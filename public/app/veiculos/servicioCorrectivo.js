$(document).ready(function () {
    ...
    const tabla = $('#servCorrec').DataTable({
        paging: true,
        ...
        "createdRow": function (row, data, index) {
            ...
            if (data.tipo_servicio == 'Correctivo') {
                $('td', row).eq(3).css({
                    'background-color': '#FF4233',
                    'color': 'black'
                })
...
const iniciarDivs = async () => {
    try {
        ...
        var table = $('#servCorrec').DataTable();
...
const obtenerDatos = async () => {
    try {
        const response = await fetch(`../../view/crud/DatosTabla.php?op=6`);
        if (!response.ok) {
