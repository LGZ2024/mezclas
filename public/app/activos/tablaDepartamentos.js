...
const obtenerRegistro = async () => {
  try {
    const response = await fetchApi('/api/catalogos/departamentos', 'GET')
...
const verDepartamentos = async () => {
  try {
    // Verificar si DataTable ya existe
    if (!$.fn.dataTable.isDataTable('#tbDepartamentos')) {
...
const inicializarEventos = () => {
  ...
  $(document).on('click', '#btnCrear', async function () {
    ...
    document.getElementById('modalDepartamentoLabel').textContent = 'Agregar Departamento'
    modal.modal('show')
  })
  ...
  $(document).on('click', '#btnEdit', async function () {
    ...
    const departamento = $(this).parents('tr').find('td:nth-child(2)').text()
  ...
  $(document).on('click', '#btnCambiarEstado', async function (e) {
    ...
    try {
      const url = `/api/catalogos/departamentos/${id}`
      const method = 'PATCH'
      const data = {
        status: status !== true
      }
...
export { iniciarDepartamentos, verDepartamentos }
