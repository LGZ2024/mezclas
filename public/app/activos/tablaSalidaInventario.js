...
const obtenerRegistro = async () => {
  try {
    const response = await fetchApi('/api/salidas', 'GET') // cambiar a GET
...
const verSalidaInventario = async () => {
  let minDate, maxDate
  try {
    ...
    if (!$.fn.dataTable.isDataTable('#tbSalidas')) {
      ...
      const tabla = $('#tbSalidas').DataTable({
        paging: true,
        orderCellsTop: true,
        fixedHeader: true,
        order: [[0, 'desc']],
...
export { iniciarSalidaInventario, verSalidaInventario }
