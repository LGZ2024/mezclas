...
const iniciarEntradaInventario = async () => {
  try {
...
const obtenerRegistro = async () => {
  try {
    const response = await fetchApi('/api/entrada', 'GET') // cambiar a GET
...
const verEntradaInventario = async () => {
  let minDate, maxDate
  try {
    ...
    if (!$.fn.dataTable.isDataTable('#tbEntrada')) {
      ...
      const tabla = $('#tbEntrada').DataTable({
        paging: true,
        orderCellsTop: true,
        fixedHeader: true,
        order: [[0, 'desc']],
...
export { iniciarEntradaInventario, verEntradaInventario }
