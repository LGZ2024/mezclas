...
const iniciarConfimaciones = async () => {
  try {
    ...
    if ($.fn.DataTable.isDataTable('#tbConfirmacion')) {
      $('#tbConfirmacion').DataTable().destroy()
      $('#tbConfirmacion tbody').empty()
    }

    // Inicializar nueva tabla con los datos
    $('#tbConfirmacion').DataTable({
