...
const iniciarMesclasV= async (empresaPertece,status) => {
    try {
        const data = await obtenerMesclaz(empresaPertece,status);
...
const verMesclazV = () => {
    const tabla = $('#tbSolicitadas').DataTable({
        ...
        columnDefs: [
            ...
            { "targets": 12, "data": "descripcion" },
            { "targets": 13, "data": "status" },
        ],
    });
}
...
/*funciones para inicializar tabla  de mezclas solicitadas*/
/*inicializar tabla solicitada*/
const iniciarMesclasVP= async (empresaPertece,status) => {
    try {
        const data = await obtenerMesclaz(empresaPertece,status);
...
/*inicializar tabla solicitada*/
const iniciarMesclasVPE= async (empresaPertece,status) => {
    try {
        const data = await obtenerMesclaz(empresaPertece,status);
...
/* obtener mezclas */
const obtenerMesclaz = async (empresaPertece,status) => {
    try {
        const response = await fechTbSolicitadas(empresaPertece,status)
        return response;
    } catch (error) {
