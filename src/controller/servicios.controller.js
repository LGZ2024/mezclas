...
export class ServicioController {
  ...
  agregarServicio = asyncHandler(async (req, res) => {
    ...
    try {
      logger.info('Iniciando Controlador Agregar Servicio', logContext)
      this.#validarDatosCreacion(req.body)
      const response = await this.servicioModel.agregarServicio({ data: req.body, logger, logContext })
      logger.info('Finalizando Controlador Agregar Servicio', logContext)
      res.json(response)
    } catch (error) {
      // Log detallado del error
      logger.error('Error al crear Servicio', {
        ...logContext,
        error: {
          name: error.name,
  ...
  obtenerServicios = asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const response = await this.servicioModel.obtenerServicios({ logger, logContext })
  ...
  obtenerUnidad = asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const response = await this.servicioModel.obtenerUnidad({ id, logger, logContext })
