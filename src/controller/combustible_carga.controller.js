...
export class CargaCombustibleController {
  constructor ({ cargaCombustibleModel }) {
    this.cargaCombustibleModel = cargaCombustibleModel
  }

  agregarCargaCombustible = asyncHandler(async (req, res) => {
    ...
    try {
      logger.info('Iniciando Controlador Agregar Carga Combustible', logContext)
      // this.#validarDatosCreacion(req.body)
      const response = await this.cargaCombustibleModel.agregarCargaCombustible({ datos: req.body, logger, logContext })
      logger.info('Finalizando Controlador Agregar Carga Combustible', logContext)
      res.json(response)
    } catch (error) {
      // Log detallado del error
      logger.error('Error al carga combustible', {
        ...logContext,
        error: {
          name: error.name,
  ...
  obtenerCargaCombustible = asyncHandler(async (req, res) => {
    const { user } = req.session
    const logger = req.logger
    const { id } = req.params
    ...
    try {
      ...
      const response = await this.cargaCombustibleModel.buscarKilometraje({ id, logger, logContext })
  ...
  obtenerCargasCombustibles = asyncHandler(async (req, res) => {
    ...
    try {
      logger.info('Iniciando Controlador Obtener Carga Combustible', logContext)
      const response = await this.cargaCombustibleModel.obtenerCargasCombustibles({ logger, logContext })
      logger.info('Finalizando Controlador Obtener Carga Combustible', logContext)
      res.json(response)
    } catch (error) {
      // Log detallado del error
